...
class ChatController extends Controller
{
    ...
    { 
        $conversations = $this->msgService->getReturningConversations()
            ->merge($this->msgService->getConversations());
    ...
    public function conversation(Website $website, $conversation_id)
    {
        ...
        $photo_album = PhotoAlbum::getPhotoAlbum($conversation['attributes']['interlocutorId']);

        if (auth()->user()->can('view', $conversation)) {
            event(new UserTakeChatEvent($website, $conversation));
            return view('chat.conversation')->with(compact('website', 'conversation', 'photo_album'));
        }
        abort(403);
    }
