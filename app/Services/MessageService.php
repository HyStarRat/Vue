    ...
    public function getConversations()
    {
        ...
        foreach ($websites as $website) {
            if (auth()->user()->can('view', $website)) {
                ...
                $collections_query = $website->managed_users()
                ...
                if(auth()->user()->country_id != null)
                    $collections_query = $collections_query->where('country_id', auth()->user()->country_id);

                $collections = $collections_query->get();

                $filtered = $collections->filter(function ($collection) {
                    return $collection->user->conversation_interlocutors ?? null;
                });
