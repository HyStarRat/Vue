<template>
	...
	<form @submit.prevent="submit()" v-if="showFakeMessageField">
		<div class="form-group" 
			v-if=" (this.auth.is_admin || this.auth.is_super )">
	...
	export default {
		...
		methods: {
			...
			submit() {
				...
				if(this.auth.is_moderator || this.auth.country_manager || this.auth.team_leader)
					data.country_id = this.auth.country_id
					
				this.$http.post('websites/' + this.website.id + '/managed-users', data).then(response => {
